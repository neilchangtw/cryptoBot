...
> 完整記錄見 [v19_research.md](v19_research.md)
...
> 完整記錄見 [v20_research.md](v20_research.md)
...
## V34：2026-08 實盤停單診斷與穩健性補測（未部署）

> 完整報告：[v34_dialogue_research.md](v34_dialogue_research.md)。本節只保留可追溯的決策摘要；所有測試均未修改 `strategy.py`、`.env` 或 VPS 設定。

### 範圍與共同基準

- 現行版本為 **V14+R + V25-D**：L15/S15、L GK `<25`、S GK `<35`、R slope `UP > 4.5%` / `SIDE < 1.0%`。
- 主要比較採 `--flat` 固定 200U；另以現行 `MARGIN_SCHEDULE`（200U → 300U@2026-07-03 → 500U@2026-08-01）交叉確認，避免把保證金變更誤認成策略差異。

### 已確認事實

| 主題 | 結果 | 判讀 |
|---|---:|---|
| 2026-08-01~12 無開單 | 272 根 1h bar 全為 `SIDE`；S 方向突破 gate 0 次；L 只有 2 個訊號候選、皆落在週末 | 並非資料或執行器故障，是市場狀態與既定 gate 的聯合結果 |
| 反事實檢查 | 移除 S slope gate：500U 模擬淨損益 **-$146.93**；L GK 放寬至 `<26`：**-$130.86** | 不能為追求近期交易數而直接放寬現行門檻 |
| 移除週末（近兩年） | 固定 200U：`+$8,048.91 → +$7,457.46`（-$591.45，-7.35%）；現行保證金排程：`+$8,081.72 → +$7,475.87`（-$605.85，-7.50%） | 週末並非只造成盤整，移除會犧牲已驗證 edge；維持現狀 |
| GK percentile 鄰域 | L `<24/25/26` × S `<34/35/36` 共 9 組，L25/S35 為該局部鄰域 PnL 冠軍（`+$8,048.91`）；81 組 GK window × threshold 網格沒有可升級候選 | 維持 L25/S35、L5/20、S10/30 |
| 延遲一根 K | Shift A（訊號於 t、成交 t+1 open）`+$6,224.83`；現行 Shift B（t 已收盤、t+1 close 執行）`+$8,048.91` | 現行模式較多 +$1,824.08；但這是現有回測成交建模下的比較，不應宣稱可保證實盤同幅度改善 |
| S `SIDE` slope 門檻 | `SIDE <1.3%` 在 5.5 年比 `<1.0%` 多 +$748、交易 385→425；但當年別 PnL 顯著集中、且有較少樣本年度 | 僅列 **shadow candidate**，不可直接上線；需依 V31 十門 gate（OOS/WF/鄰域/成本/時序）重新稽核 |

### 2,000 天長樣本（固定 200U）

資料為 2021-02-19 至 2026-08-12，共 47,999 根 1h bar：673 筆、淨損益 **+$10,092.26**、勝率 57.65%、PF 1.68、MDD $1,071.67。2021 與 2023 明顯偏弱，主因是 ETH 的壓縮後突破延續不足、MaxHold 虧損占比提高；這和 V14 已知的 regime dependency 一致，不能解讀為保證未來會重演。

### V29 健康警報重申

- 健康度只在出場後更新；🟡 `≤25%` 為凍結加碼提醒、🔴 `0%` 為退回 200U 提醒。
- 現行實作是 **通知＋人工執行**，不是自動停單、也不是已寫死的 200U／100U 降檔。
- 若要自動化，應先將「風險降檔」與「進場封鎖」分成兩個可獨立回測的 policy，並先做 shadow log；本輪沒有授權部署。
