...
class Tests(unittest.TestCase):
    ...
    def test_saved_failure_never_requests_network(self):
        with tempfile.TemporaryDirectory() as tmp:
            p=Path(tmp); (p/'source_failure.json').write_text('{}')
            with patch.object(r,'DOC',p),patch.object(r.requests,'Session') as network:
                with self.assertRaises(RuntimeError): r.request_once(0,1,'forbidden_retry')
