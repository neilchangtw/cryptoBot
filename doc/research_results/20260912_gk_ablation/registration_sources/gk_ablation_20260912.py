...
def register():
    ...
    if (DOC/'registration.json').exists():
        reg=read(DOC/'registration.json');assert inputs==reg['sha256'];a.prior_audit.verify_map(reg['protected_sha256']);return
    a.register()
...
def evaluate():
    ...
    result={'status':'REJECTED' if any(failures.values()) else 'BASIC_PASS_REQUIRES_VALIDATION','counts':diag['counts'],
        ...
        'controls_evaluated':0,'download_bytes':0,'preserved_files':a.prior_audit.verify_map(read(DOC/'registration.json')['protected_sha256']),
