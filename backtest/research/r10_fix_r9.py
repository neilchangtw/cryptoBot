"""
R10 Fix Round 9: SN4.5% + MT12 + PT35
========================================
R8 (PT35+MT8) = 8/9, great monthly (60%), good OOS ($5,724), but IS -$1,099.
R7 (PT30+MT8) = 8/9, IS -$899, monthly 56%.
R1 (PT30+MT12) had IS -$931 (better than MT8's -$953).

Hypothesis: PT=35 + MT=12.
- PT35 for monthly smoothing (R8 showed 60% positive months)
- MT12 for better IS PnL (MT12 was ~$20 better than MT8 in R1 vs R3)
- The extra trades from PT35 might smooth months enough to keep consec<=3
  even with MT12 (R1 had consec=4 with PT30+MT12 because fewer trades)
...
[Y] fix direction decided before data?  PT35 from R8 validation, MT12 from R1 IS finding
[Y] no post-result parameter adjustment?  Combining two pre-validated improvements
[Y] no OOS leakage?  No new indicators
"""
...
# ★ Round 9
SAFENET_PCT = 0.045
MIN_TRAIL = 12            # ★ back to 12 (better IS PnL)
PARK_THRESH = 35          # ★ 35 (better monthly consistency)
...
def backtest(df):
    ...
    sn=SAFENET_PCT; mt=MIN_TRAIL; pt=PARK_THRESH
    lp=[]; sp=[]; tr=[]
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 9: SN4.5% + MT12 + PT35")
    print("="*70)
    ...
    print(f"  IS SafeNet: {len(sn_is)} (${sn_is['pnl'].sum():+,.0f}) / Trail: {len(tr_is)} (${tr_is['pnl'].sum():+,.0f})")
    print(f"  OOS SafeNet: {len(sn_oos)} (${sn_oos['pnl'].sum():+,.0f}) / Trail: {len(tr_oos)} (${tr_oos['pnl'].sum():+,.0f})")
    ...
    print(f"\n  +{'='*62}+")
    print(f"  | R10-R9: SN4.5% + MT12 + PT35                                 |")
