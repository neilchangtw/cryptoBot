"""
Round 18: Triple OR Ensemble (GK + Skewness + Return Sign)
===========================================================
Hypothesis: Different types of market preparation precede valid breakouts.
OR logic EXPANDS entries (not filters). Three orthogonal dimensions:
  1. GK compression: volatility quieting (2nd moment of price)
...
Parameters: ALL pre-fixed from proven strategies, zero adjustment.
  GK: mean(5)/mean(20), pctile(100) < 30 [Champion locked params]
  Skewness: rolling(20).skew().shift(1), thresh=1.0 [R1 proven]
  Return Sign: (ret>0).rolling(15).mean().shift(1), thresh=0.60 [R16 proven]

Entry: (GK_comp OR skew_extreme OR sign_extreme) AND breakout AND session
       AND fresh AND CD12 AND maxSame=4
  GK: non-directional compression + breakout determines direction
  Skew: skew>1.0 + breakout long / skew<-1.0 + breakout short
  Sign: sr>0.60 + breakout long / sr<0.40 + breakout short
"""
...
def calc(df):
    ...
    # --- Signal 1: GK Compression (Champion exact formula) ---
    ...
    # Long: any of the three signals could trigger
    # GK fires for both directions, skew/sign are directional
    d["any_l"]=d["gk_comp"]|d["skew_l"]|d["sr_l"]  # any long trigger at bar i
    d["any_s"]=d["gk_comp"]|d["skew_s"]|d["sr_s"]  # any short trigger at bar i
    d["al_p"]=d["any_l"].shift(1)  # T-2: any long trigger at bar i-1
...
def bt(df):
    W=GK_LONG+GK_WIN+20  # GK dominates warmup
    ...
    for i in range(W,len(df)-1):
        ...
        # --- Entry signals (OR logic) ---
        # Get signal values, handle NaN
        gkc=_b(GKC[i])
        ...
        # OR logic for long: (GK_comp OR skew_long OR sr_long) AND breakout_long
        long_sig = (gkc or skl or srl) and bl and sok
        short_sig = (gkc or sks or srs) and bs and sok

        # Freshness: NOT (any_signal_at_T-2 AND breakout_at_T-2 AND session_at_T-2)
        fl = not(_b(ALP[i]) and _b(BLP[i]) and _b(SOKP[i]))
        fs = not(_b(ASP[i]) and _b(BSP[i]) and _b(SOKP[i]))

        # Determine which signal(s) triggered (for analysis)
...
def report(title,tdf,months):
    ...
    if "sig" in tdf.columns:
        ...
        for sig_tag in sorted(tdf["sig"].unique()):
            ...
            print(f"    {sig_tag:<12s}: {n_:>4d}t ${p_:>+8,.0f}")
...
if __name__=="__main__":
    print("="*60)
    print("  Round 18: Triple OR Ensemble (GK + Skew + RetSign)")
    print("="*60)
    df=load();df=calc(df)
    ...
    print(f"    [3] All rolling have shift(1)?: YES: gk_r.shift(1), skew.shift(1), sr.shift(1)")
    print(f"    [4] Params pre-fixed?: YES: GK from Champion, Skew from R1, Sign from R16")
