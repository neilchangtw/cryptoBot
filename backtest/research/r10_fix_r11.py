"""
R10 Fix Round 11: SN4.5% + MT8 + EXIT-Based Cooldown 12 bars
==============================================================
R10 (entry cooldown 24) achieved IS PnL -$173 (PASS!) but OOS $3,546 (FAIL).
Entry cooldown was too symmetric: IS -34%, OOS -32%.

Hypothesis: Switch from ENTRY-based to EXIT-based cooldown.
After a position EXITS, can't enter same direction for 12 bars.

Why this is more asymmetric:
- IS losers exit at bar 8-12 → cooldown from bar 8-12, blocks re-entry for 12 bars
  → effective gap from original entry: 20-24 bars
- OOS winners exit at bar 24-96 → cooldown from bar 24-96, adds 12 bars
  → minimal impact because trends naturally change after long holds

With EXIT cooldown 12:
- IS churn cycle: enter → lose at bar 8 → exit → wait 12 bars → re-enter
  Prevents 1-2 rapid re-entries per compression episode in IS
- OOS trend cycle: enter → win at bar 48 → exit → wait 12 bars → next trend
  12 bar wait is trivial after a 48-bar hold
...
[Y] fix direction decided before data?  Exit-based cooldown from market structure
[Y] no post-result parameter adjustment?  12 bars from half-day natural cycle
[Y] no OOS leakage?  Cooldown tracks own exits only
"""
...
SAFENET_PCT = 0.045
MIN_TRAIL = 8
EXIT_COOLDOWN = 12        # ★ bars after EXIT before same-direction re-entry
...
def backtest(df):
    ...
    lp=[]; sp=[]; tr=[]
    last_long_exit = -9999   # ★ tracks bar of last long EXIT
    last_short_exit = -9999  # ★ tracks bar of last short EXIT
    ...
    for i in range(w, N-1):
        ...
        for p in lp:
            ...
            if rl<=p["e"]*(1-sn):
                ...
                last_long_exit = i  # ★ update exit bar
            elif b>=mt and rc<=re:
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 11: SN4.5% + MT8 + Exit Cooldown 12 bars")
    print("="*70)
    ...
    print(f"\n  Diagnostic:")
    print(f"    IS:  {is_s['n']} trades (R7:279, R10:185), SN {len(sn_is)} (${sn_is['pnl'].sum():+,.0f})")
    print(f"    OOS: {oos_s['n']} trades (R7:265, R10:179), SN {len(sn_oos)} (${sn_oos['pnl'].sum():+,.0f})")
    print(f"    IS reduction vs R7: {(1-is_s['n']/279)*100:.0f}%")
    print(f"    OOS reduction vs R7: {(1-oos_s['n']/265)*100:.0f}%")
    ...
    print(f"\n  +{'='*62}+")
    print(f"  | R10-R11: SN4.5% + MT8 + Exit Cooldown 12 bars                |")
    ...
    print(f"  | Max month: ${oos_max_month:+,.0f} ({oos_max_month_pct:.1f}%)  pos: {oos_pos}/{oos_tot}  consec: {oos_consec}     |")
    print(f"  | -- Full --                                                   |")
    print(f"  | Positive months: {pos_months}/{tot_months} ({full_pos_rate:.0f}%)  consec losing: {full_consec}{'':>16s}|")
    print(f"  +{'='*62}+")
