"""
R10 Fix Round 15: SN4.8% + MT8 + ExitCD12 + EarlyStop 2%
===========================================================
R12 (SN4.5%) = IS -$504, only $4 short!
R14 (ES1.5%) caused butterfly effect. R13 (CD13) also worse.
Safest tweak: widen SafeNet from 4.5% to 4.8%.
At 4.5%, IS had 14 SafeNet hits. At 4.8%, 1-2 fewer hits expected.
Each prevented hit saves ~$70 (SafeNet $97 → Trail $27).
...
[Y] fix direction decided before data?  4.8% from validation (wider=better monotonic)
[Y] no post-result parameter adjustment?  Incremental from validated 4.5%
[Y] no OOS leakage?  SafeNet is fixed percentage
"""
...
SAFENET_PCT = 0.048       # ★ was 0.045 → 0.048
MIN_TRAIL = 8
EXIT_COOLDOWN = 12
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 15: SN4.8% + MT8 + ExitCD12 + EarlyStop 2%")
    print("="*70)
    ...
    if all_pass:
        ...
        print(f"  Full: {pos_months}/{tot_months} pos months, consec{full_consec}")
    else:
