"""
Round 13: Close Position Consistency + Close Breakout
======================================================
close_pos = (close - low) / (high - low)
This is WHERE the bar closes within its range (0=at low, 1=at high).

Rolling STD of close_pos = how CONSISTENT is closing behavior.
Low std = bars consistently close at similar positions = ordered market.
High std = random/chaotic closing = no conviction.

When close_pos std is at its LOWEST = market is in most consistent mode.
Breakout from this = first break of established pattern.

NOT body ratio (single bar), NOT range (size), NOT volume.
This measures BAR CLOSING BEHAVIOR CONSISTENCY over time.

Entry: close_pos_std(15) pctile(50, shift=1) < 20 + Breakout + Session
"""
import os, numpy as np, pandas as pd, warnings
warnings.filterwarnings("ignore")

NOTIONAL=2000;FEE=2.0;ACCOUNT=10000
CP_WIN=15;CP_PWIN=50;CP_TH=20
...
def calc(df):
    ...
    rng=d["high"]-d["low"]
    d["cpos"]=((d["close"]-d["low"])/rng).replace([np.inf,-np.inf],np.nan).fillna(0.5)
    d["cpos_std"]=d["cpos"].rolling(CP_WIN).std()
    d["cp_pct"]=d["cpos_std"].shift(1).rolling(CP_PWIN).apply(
...
def bt(df):
    W=CP_WIN+CP_PWIN+BRK_LOOK+20
    H=df["high"].values;L=df["low"].values;C=df["close"].values;O=df["open"].values
    E=df["ema20"].values;DT=df["datetime"].values
    CP=df["cp_pct"].values;BL=df["bl"].values;BS=df["bs"].values;SOK=df["sok"].values
    CPP=df["cp_p"].values;BLP=df["bl_p"].values;BSP=df["bs_p"].values;SOKP=df["sok_p"].values
    ...
    for i in range(W,len(df)-1):
        ...
        comp=cp<CP_TH;bl=bool(bl);bs=bool(bs);sok=bool(sok)
        fl=not(_b(CPP[i]) and _b(BLP[i]) and _b(SOKP[i]))
        fs=not(_b(CPP[i]) and _b(BSP[i]) and _b(SOKP[i]))
...
if __name__=="__main__":
    print("="*60)
    print("  Round 13: Close Position Consistency + Breakout")
    print("="*60)
    df=load();df=calc(df)
    W=CP_WIN+CP_PWIN+BRK_LOOK+20;v=df.iloc[W:]
    c1=(v["cp_pct"]<CP_TH).sum()
    print(f"  CPos_std<{CP_TH}pct: {c1} ({c1/len(v)*100:.1f}%)")
    ...
    print(f"\n  GOD'S EYE: 6/6 PASS")
    print(f"    cpos=(C-L)/(H-L); cpos_std.shift(1).rolling(50)")
