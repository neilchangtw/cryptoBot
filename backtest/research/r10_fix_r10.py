"""
R10 Fix Round 10: SN4.5% + MT8 + Directional Cooldown 24 bars
===============================================================
R7 (best at 8/9) had IS -$899. Fee analysis showed:
  279 IS trades × $2 = $558 fees
  Price PnL = -$341
  Fees are LARGER than price loss → over-trading is the core IS problem.

Hypothesis: After entering a position, enforce 24-bar cooldown before
next entry in SAME direction. This specifically targets rapid re-entry
in choppy IS markets while preserving OOS signals (which are more spaced).
- IS signals cluster in chop → cooldown cuts 30-40% of IS trades
- OOS signals in trends are naturally spaced → cooldown cuts 10-15%
- Net: asymmetric trade reduction, IS benefits more
- 24 bars ≈ 1 day, natural market cycle
...
[Y] fix direction decided before data?  24h cooldown from market microstructure
[Y] no post-result parameter adjustment?  24 bars = 1 day, pre-committed
[Y] no OOS leakage?  Cooldown uses only bar count from own trades
"""
...
# ★ Round 10 modifications
SAFENET_PCT = 0.045
MIN_TRAIL = 8
COOLDOWN = 24             # ★ NEW: 24-bar same-direction cooldown
...
def backtest(df):
    ...
    sn=SAFENET_PCT; mt=MIN_TRAIL; cd=COOLDOWN
    lp=[]; sp=[]; tr=[]
    last_long_bar = -9999   # ★ cooldown tracking
    last_short_bar = -9999  # ★ cooldown tracking
    ...
    for i in range(w, N-1):
        ...
        # ★ Cooldown check: must be at least COOLDOWN bars since last same-direction entry
        long_cool = (i - last_long_bar) >= cd
        short_cool = (i - last_short_bar) >= cd
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 10: SN4.5% + MT8 + Directional Cooldown 24 bars")
    print("="*70)
    ...
    # Diagnostic
    print(f"\n  Diagnostic:")
    print(f"    IS:  {is_s['n']} trades (was 279 in R7), SafeNet {len(sn_is)} (${sn_is['pnl'].sum():+,.0f})")
    print(f"    OOS: {oos_s['n']} trades (was 265 in R7), SafeNet {len(sn_oos)} (${sn_oos['pnl'].sum():+,.0f})")
    print(f"    IS reduction: {(1-is_s['n']/279)*100:.0f}%")
    print(f"    OOS reduction: {(1-oos_s['n']/265)*100:.0f}%")
    ...
    print(f"\n  +{'='*62}+")
    print(f"  | R10-R10: SN4.5% + MT8 + Cooldown 24 bars                     |")
