"""
R10 Fix Round 3: SafeNet 4.5% + MIN_TRAIL 8 bars
==================================================
R1: SafeNet 4.5% improved OOS (+$507) and IS (+$215), but IS still -$931.
R2: Breakout strength filter was catastrophic — halved trade count, halved OOS PnL.
    Entry filtering kills the strategy; stop trying to filter entries.

Hypothesis: Reduce MIN_TRAIL from 12 to 8 bars.
- 12-24h bucket is the biggest loss driver (OOS: 67 trades, -$974, 25% WR)
- Reducing min hold lets EMA20 trail kick in 4 bars earlier
- Losing trades that drift below EMA20 by bar 8 exit sooner → smaller loss per trade
- Winners (24-48h+) are strongly trending → rarely cross back below EMA20 at bar 8-12
- SafeNet 4.5% kept from R1 (widened stop proven effective)

Self-check (pre-committed BEFORE running):
[Y] signal only uses shift(1)+?  Same as R10, no new indicators
[Y] entry price = next bar open?  opens[i+1]
[Y] all rolling indicators have shift(1)?  Same as R10
[Y] fix direction decided before data?  MIN_TRAIL reduction from ETH hold-time physics
[Y] no post-result parameter adjustment?  8 bars = 1/3 of typical day, pre-committed
[Y] no OOS leakage?  No new indicators, same framework
"""
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 3: SafeNet 4.5% + MIN_TRAIL 8 bars")
    print("="*70)
    ...
    # ── Output ──────────────────────────────────────────────────────────
    print(f"\n  +{'='*62}+")
    print(f"  | R10-R3: SafeNet 4.5% + MIN_TRAIL 8 bars                      |")
    ...
    # Comparison with R10 baseline and R1
    print(f"\n  Comparison:")
    print(f"  {'Metric':<28s} {'R10':>10s} {'R1(SN45)':>10s} {'R3(+MT8)':>10s}")
    print(f"  {'-'*62}")
    cmp=[
        ("OOS PnL","$+4,858","$+5,365",f"${oos_s['pnl']:>+,.0f}"),
        ("OOS PF","1.96","2.30",f"{oos_s['pf']:.2f}"),
        ("OOS MDD","4.6%","4.0%",f"{oos_s['mdd_pct']:.1f}%"),
        ("IS PnL","$-1,146","$-931",f"${is_s['pnl']:>+,.0f}"),
        ("IS trades","275","275",f"{is_s['n']}"),
        ("OOS trades","261","261",f"{oos_s['n']}"),
        ("Full pos months","12/25 (48%)","13/25 (52%)",f"{pos_months}/{tot_months} ({full_pos_rate:.0f}%)"),
        ("Max consec losing","4","4",f"{full_consec}"),
        ("OOS max month%","34.7%","32.6%",f"{oos_max_month_pct:.1f}%"),
        ("OOS monthly avg","21.8","21.8",f"{oos_monthly_avg:.1f}"),
    ]
    for label,r10,r1,r3 in cmp:
        print(f"  {label:<28s} {r10:>10s} {r1:>10s} {r3:>10s}")
