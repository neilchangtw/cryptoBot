"""
V10-L Round 8: TP + MaxHold Exit (Short-style for Long)

R7 diagnosis:
  MDD <= $400 -> 0/1728 configs PASS (best MDD=$414)
  Root cause: EMAcross exit WR=22-28%, most trend-following trades lose
  SafeNet hits ($150-230 each) stack into MDD during bad months

Paradigm shift: Instead of riding trends with EMAcross, use TP + MaxHold
like the Short strategy. Cap both upside and downside per trade.

R4 exit analysis (best config):
  MaxHold:  18t avg $+287 (WR 100%)  -- time limit = profitable
  EMAcross: 53t avg $-19  (WR 28%)   -- trend riding mostly fails
  SafeNet:   8t avg $-190 (WR 0%)    -- big losses from stops

With TP + MaxHold, we capture partial moves with higher WR,
eliminating the EMAcross drag entirely.

Grid: GK x BL x TP x MaxHold x SN x mCap x cd = 1,296 configs
"""
import sys, io
...
def run_bt(df, p, start, end):
    ...
    for i in range(start, end):
        ...
        if pos is not None:
            ...
            # SafeNet (highest priority -- worst case)
        ...
        # Entry: GK compression + close breakout up
        gk_val = b["gk_pct"]
...
# --- Build configs ---
configs = []
for gk_t in [20, 30, 40]:
    for bl in [10, 12, 15]:
        for tp in [1.0, 1.5, 2.0, 3.0]:
            for maxh in [5, 8, 12]:
                for sn in [2.0, 3.0, 4.0]:
                    for m_cap in [-100, -150]:
                        for cd in [8, 12]:
                            configs.append({
...
print(f"=== V10-L R8: TP + MaxHold Exit ===")
print(f"GK breakout + TP + MaxHold (Short-style for Long)")
...
for tp in [1.0, 1.5, 2.0, 3.0]:
    ...
    print(f"\n[TP={tp}%] IS>0: {is_pos}/{len(sub)} | Both>0: {len(both)} | Goals PASS: {len(goal)}")
    if best:
        o=best["oos"]; s=best["is"]; p=best["p"]
        pm=f"{o['pos_m']}/{o['tot_m']}"
        print(f"  Best: GK<{p['gk_thresh']} BL={p['bl']} SN={p['safenet']} maxH={p['max_hold']} "
              f"mCap={p['m_loss_cap']} cd={p['exit_cd']}")
        print(f"  IS: {s['n']}t ${s['pnl']:+.0f} WR={s['wr']:.0f}% | "
              f"OOS: {o['n']}t ${o['pnl']:+.0f} WR={o['wr']:.0f}% PF={o['pf']:.2f} "
              f"MDD=${o['mdd']:.0f} PM={pm} WM=${o['worst_m']:+.0f}")

print(f"\n{'='*65}")
print("RESULTS BY MAXHOLD LEVEL")
print(f"{'='*65}")
...
for mxh in [5, 8, 12]:
    ...
    is_pos = sum(1 for r in sub if r["is_pnl"]>0)
    best = max(both, key=lambda x:x["oos_pnl"]) if both else None
    print(f"\n[maxH={mxh}] IS>0: {is_pos}/{len(sub)} | Both>0: {len(both)} | Goals PASS: {len(goal)}")
    if best:
        o=best["oos"]; s=best["is"]; p=best["p"]
        pm=f"{o['pos_m']}/{o['tot_m']}"
        print(f"  Best: GK<{p['gk_thresh']} BL={p['bl']} SN={p['safenet']} TP={p['tp']} "
              f"mCap={p['m_loss_cap']} cd={p['exit_cd']}")
        print(f"  IS: {s['n']}t ${s['pnl']:+.0f} WR={s['wr']:.0f}% | "
              f"OOS: {o['n']}t ${o['pnl']:+.0f} WR={o['wr']:.0f}% PF={o['pf']:.2f} "
              f"MDD=${o['mdd']:.0f} PM={pm} WM=${o['worst_m']:+.0f}")
...
for sn in [2.0, 3.0, 4.0]:
    ...
    if best:
        o=best["oos"]; s=best["is"]; p=best["p"]
        pm=f"{o['pos_m']}/{o['tot_m']}"
        print(f"  Best: GK<{p['gk_thresh']} BL={p['bl']} TP={p['tp']} maxH={p['max_hold']} "
              f"mCap={p['m_loss_cap']} cd={p['exit_cd']}")
        print(f"  IS: {s['n']}t ${s['pnl']:+.0f} WR={s['wr']:.0f}% | "
              f"OOS: {o['n']}t ${o['pnl']:+.0f} WR={o['wr']:.0f}% PF={o['pf']:.2f} "
              f"MDD=${o['mdd']:.0f} PM={pm} WM=${o['worst_m']:+.0f}")
...
for rank, r in enumerate(all_both[:3], 1):
    ...
    print(f"\n{'~'*60}")
    print(f"[#{rank}] GK<{p['gk_thresh']} BL={p['bl']} TP={p['tp']}% "
...
print("R8: GK Breakout+TP+MaxHold       -> see above")
