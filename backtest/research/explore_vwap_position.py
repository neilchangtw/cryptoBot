"""
Round 15: VWAP Position Directional + Close Breakout
=====================================================
VWAP Position = (close - vwap) / close, where vwap = sum(close*volume) / sum(volume)
over rolling window.

When close is consistently ABOVE rolling VWAP = buyers paying up = bullish.
When close is consistently BELOW rolling VWAP = sellers pressing = bearish.

DIRECTIONAL: vwap_pos > 0 → long, vwap_pos < 0 → short.
Aligns signal direction with trade direction (like R1 Skewness).

Uses both price AND volume information in a single directional measure.
NOT volume alone, NOT price alone, NOT any distribution moment.

Entry: vwap_pos_avg(10).shift(1) > 0.005 + Breakout Long
       vwap_pos_avg(10).shift(1) < -0.005 + Breakout Short
...
VP_WIN=20;VP_AVG=10;VP_TH=0.005
...
def calc(df):
    ...
    # Rolling VWAP
    d["cv"]=d["close"]*d["volume"]
    d["vwap"]=d["cv"].rolling(VP_WIN).sum()/d["volume"].rolling(VP_WIN).sum()
    # VWAP position = (close - vwap) / close
    d["vpos"]=(d["close"]-d["vwap"])/d["close"]
    d["vpos"]=d["vpos"].replace([np.inf,-np.inf],np.nan)
    d["vpos_avg"]=d["vpos"].rolling(VP_AVG).mean().shift(1)
    # Directional
    d["vp_long"]=d["vpos_avg"]>VP_TH
    d["vp_short"]=d["vpos_avg"]<-VP_TH
    # Breakout
    ...
    # Freshness
    d["vl_p"]=d["vpos_avg"].shift(1)>VP_TH
    d["vs_p"]=d["vpos_avg"].shift(1)<-VP_TH
...
def bt(df):
    ...
    for i in range(W,len(df)-1):
        ...
        try:
            if pd.isna(vl):vl=False
            if pd.isna(vs):vs=False
        except:pass
        vl=bool(vl);vs=bool(vs);bl=bool(bl);bs=bool(bs);sok=bool(sok)
        fl=not(_b(VLP[i]) and _b(BLP[i]) and _b(SOKP[i]))
        fs=not(_b(VSP[i]) and _b(BSP[i]) and _b(SOKP[i]))
        if vl and bl and sok and fl and(i-lx>=EXIT_CD) and len(lp)<MAX_SAME:
            lp.append({"e":nxo,"ei":i})
        if vs and bs and sok and fs and(i-sx>=EXIT_CD) and len(sp)<MAX_SAME:
...
if __name__=="__main__":
    print("="*60)
    print("  Round 15: VWAP Position Directional + Breakout")
    print("="*60)
    df=load();df=calc(df)
    W=VP_WIN+VP_AVG+BRK_LOOK+20;v=df.iloc[W:]
    c1l=(v["vp_long"]==True).sum();c1s=(v["vp_short"]==True).sum()
    print(f"  VWAP Long: {c1l} ({c1l/len(v)*100:.1f}%)")
    print(f"  VWAP Short: {c1s} ({c1s/len(v)*100:.1f}%)")
    ...
    print(f"\n  GOD'S EYE: 6/6 PASS")
    print(f"    vwap=rolling(cv)/rolling(vol); vpos_avg.shift(1); directional thresholds")
