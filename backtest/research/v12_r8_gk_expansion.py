"""V12 R8: GK Expansion Reversal Short
Opposite of GK compression breakout: when GK percentile is HIGH (expansion),
price tends to mean-revert. Short when volatile + price above MA.

Also test: GK expansion + specific bearish signals
"""
import sys, io
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace', line_buffering=True)

import pandas as pd
import numpy as np

df = pd.read_csv("data/ETHUSDT_1h_latest730d.csv")
df["datetime"] = pd.to_datetime(df["datetime"])

NOTIONAL = 4000; FEE = 4.0; WARMUP = 200
TOTAL = len(df); IS_END = TOTAL // 2
CONSEC_LOSS_PAUSE = 4; CONSEC_LOSS_COOLDOWN = 24

c = df["close"].astype(float); h = df["high"].astype(float)
l = df["low"].astype(float); o = df["open"].astype(float)
v = df["volume"].astype(float)

# Session
df["hour"] = df["datetime"].dt.hour; df["dow"] = df["datetime"].dt.dayofweek
df["session_ok"] = ~(df["hour"].isin([0,1,2,12]) | df["dow"].isin([0,5,6]))

# --- GK pctile ---
ln_hl = np.log(h / l); ln_co = np.log(c / o)
gk = 0.5 * ln_hl**2 - (2 * np.log(2) - 1) * ln_co**2
gk_ratio = gk.rolling(5).mean() / gk.rolling(20).mean()
df["gk_pct"] = gk_ratio.shift(1).rolling(100).apply(
    lambda s: ((s < s.iloc[-1]).sum()) / (len(s)-1) * 100 if len(s)>1 else 50, raw=False)

# --- EMA deviation ---
ema20 = c.ewm(span=20).mean()
df["ema_dev"] = ((c - ema20) / ema20 * 100).shift(1)

# --- Breakout down (V11-E S baseline) ---
df["brk_dn_15"] = c < c.shift(1).rolling(15).min()

# --- SMA50 deviation ---
sma50 = c.rolling(50).mean()
df["sma50_dev"] = ((c - sma50) / sma50 * 100).shift(1)

# --- Bearish candle ---
df["bearish"] = (c < o).shift(1)

# --- Bearish close: close in lower 30% of range ---
df["close_low"] = ((c - l) / (h - l + 1e-10) < 0.3).shift(1)

# --- Volume spike ---
vol_ma20 = v.rolling(20).mean()
df["vol_mult"] = (v / vol_ma20).shift(1)

# --- Price change ---
df["pchg"] = ((c / c.shift(1) - 1) * 100).shift(1)

# --- RSI ---
delta = c.diff()
gain = delta.clip(lower=0).rolling(14).mean()
loss_ = (-delta.clip(upper=0)).rolling(14).mean()
rs = gain / loss_
df["rsi14"] = (100 - 100 / (1 + rs)).shift(1)

arr_o = o.values; arr_h = h.values; arr_l = l.values; arr_c = c.values
arr_dt = df["datetime"].values; arr_sess = df["session_ok"].values


def run_short_generic(entry_mask, tp_pct, mh, sn_pct, cd, mcap, start, end):
    tp = tp_pct / 100; sn = sn_pct / 100
    trades = []; in_pos = False; ep = 0.0; idx = 0
    last_exit = -999; d_pnl = m_pnl = 0.0; consec = 0; cd_until = 0
    cur_m = ""; cur_d = ""; m_ent = 0

    for i in range(start, end):
        dt = pd.Timestamp(arr_dt[i])
        m_str = str(dt.to_period("M")); d_str = str(dt.date())
        if d_str != cur_d: d_pnl = 0.0; cur_d = d_str
        if m_str != cur_m: m_pnl = 0.0; m_ent = 0; cur_m = m_str

        if in_pos:
            held = i - idx; xp = 0.0; xr = ""
            sn_level = ep * (1 + sn)
            if arr_h[i] >= sn_level:
                xp = sn_level + (arr_h[i] - sn_level) * 0.25; xr = "SafeNet"
            if xr == "" and arr_l[i] <= ep * (1 - tp):
                xp = ep * (1 - tp); xr = "TP"
            if xr == "" and held >= mh:
                xp = arr_c[i]; xr = "MaxHold"
            if xr:
                pnl = (1 - xp/ep) * NOTIONAL - FEE
                trades.append({"pnl": pnl, "reason": xr, "month": m_str, "held": held})
                d_pnl += pnl; m_pnl += pnl
                if pnl < 0:
                    consec += 1
                    if consec >= CONSEC_LOSS_PAUSE: cd_until = i + CONSEC_LOSS_COOLDOWN
                else: consec = 0
                last_exit = i; in_pos = False

        if in_pos: continue
        if d_pnl <= -200 or m_pnl <= mcap or i < cd_until: continue
        if i - last_exit < cd or m_ent >= 20: continue
        if not arr_sess[i]: continue

        if entry_mask[i] and i + 1 < end:
            ep = arr_o[i+1]; idx = i+1; in_pos = True; m_ent += 1
    return trades


def eval_config(entry_mask, tp_pct, mh, sn_pct=4.0, cd=8, mcap=-150):
    is_t = run_short_generic(entry_mask, tp_pct, mh, sn_pct, cd, mcap, WARMUP, IS_END)
    is_pnl = sum(t["pnl"] for t in is_t)
    if is_pnl <= 0 or len(is_t) < 5:
        return None

    oos_t = run_short_generic(entry_mask, tp_pct, mh, sn_pct, cd, mcap, IS_END, TOTAL)
    oos_pnl = sum(t["pnl"] for t in oos_t)
    is_wr = sum(1 for t in is_t if t["pnl"]>0)/len(is_t)*100
    oos_wr = sum(1 for t in oos_t if t["pnl"]>0)/len(oos_t)*100 if oos_t else 0

    oos_m = pd.DataFrame(oos_t).groupby("month")["pnl"].sum() if oos_t else pd.Series(dtype=float)
    pm = sum(1 for m in oos_m.index if oos_m[m] > 0)
    worst = oos_m.min() if len(oos_m) else -999

    return {
        "is_n": len(is_t), "is_pnl": is_pnl, "is_wr": is_wr,
        "oos_n": len(oos_t), "oos_pnl": oos_pnl, "oos_wr": oos_wr,
        "pm": pm, "tot_m": len(oos_m), "worst": worst,
        "tp_pct": tp_pct, "mh": mh,
    }


def print_top(name, results, col_w=45):
    results.sort(key=lambda x: x["oos_pnl"], reverse=True)
    print(f"\nIS>0 configs: {len(results)}")
    if not results: return
    print(f"\n--- Top 15 ---")
    print(f"{'Config':>{col_w}} | {'IS_n':>4} {'IS':>7} {'WR':>5} | {'OOS_n':>5} {'OOS':>7} {'WR':>5} | {'PM':>5} {'WM':>6}")
    for r in results[:15]:
        print(f"{r['cfg']:>{col_w}} | {r['is_n']:>4} {r['is_pnl']:>+7.0f} {r['is_wr']:>5.1f} | {r['oos_n']:>5} {r['oos_pnl']:>+7.0f} {r['oos_wr']:>5.1f} | {r['pm']:>2}/{r['tot_m']:>2} {r['worst']:>+6.0f}")


# ===== GK Expansion Simple =====
print("="*70)
print("  GK Expansion Simple: GK pctile > threshold → Short")
print("="*70)

gk_vals = df["gk_pct"].values
results_gke = []
for gk_min in [50, 60, 70, 75, 80, 85, 90]:
    for tp in [1.5, 2.0, 2.5, 3.0]:
        for mh in [5, 6, 7, 8, 10]:
            mask = np.array([gk_vals[i] == gk_vals[i] and gk_vals[i] >= gk_min for i in range(len(df))])
            r = eval_config(mask, tp, mh)
            if r:
                r["cfg"] = f"GK>={gk_min} tp{tp} mh{mh}"
                results_gke.append(r)

print_top("GK Expansion Simple", results_gke)


# ===== GK Expansion + Price Above EMA =====
print("\n" + "="*70)
print("  GK Expansion + Price Above EMA20 → Short")
print("="*70)

ed_vals = df["ema_dev"].values
results_gke2 = []
for gk_min in [50, 60, 70, 80]:
    for ed_min in [0, 1.0, 2.0, 3.0]:
        for tp in [1.5, 2.0, 2.5, 3.0]:
            for mh in [5, 6, 7, 8, 10]:
                mask = np.array([False] * len(df))
                for i in range(len(df)):
                    g = gk_vals[i]; e = ed_vals[i]
                    if g==g and e==e and g>=gk_min and e>=ed_min:
                        mask[i] = True
                r = eval_config(mask, tp, mh)
                if r:
                    r["cfg"] = f"GK>={gk_min}+EMA>={ed_min} tp{tp} mh{mh}"
                    results_gke2.append(r)

print_top("GK Expansion + EMA", results_gke2)


# ===== GK Expansion + Bearish Confirmation =====
print("\n" + "="*70)
print("  GK Expansion + Bearish Candle → Short")
print("="*70)

bear_vals = df["bearish"].fillna(False).values
cl_vals = df["close_low"].fillna(False).values
results_gke3 = []
for gk_min in [50, 60, 70, 80]:
    for bear_type in ["bearish", "close_low"]:
        bv = bear_vals if bear_type == "bearish" else cl_vals
        for tp in [1.5, 2.0, 2.5, 3.0]:
            for mh in [5, 6, 7, 8, 10]:
                mask = np.array([False] * len(df))
                for i in range(len(df)):
                    g = gk_vals[i]
                    if g==g and g>=gk_min and bv[i]:
                        mask[i] = True
                r = eval_config(mask, tp, mh)
                if r:
                    r["cfg"] = f"GK>={gk_min}+{bear_type} tp{tp} mh{mh}"
                    results_gke3.append(r)

print_top("GK Expansion + Bearish", results_gke3)


# ===== GK Expansion + Volume Spike =====
print("\n" + "="*70)
print("  GK Expansion + Volume Spike → Short")
print("="*70)

vm_vals = df["vol_mult"].values
results_gke4 = []
for gk_min in [50, 60, 70, 80]:
    for vm_min in [1.5, 2.0, 2.5]:
        for tp in [1.5, 2.0, 2.5, 3.0]:
            for mh in [5, 6, 7, 8]:
                mask = np.array([False] * len(df))
                for i in range(len(df)):
                    g = gk_vals[i]; vm_i = vm_vals[i]
                    if g==g and vm_i==vm_i and g>=gk_min and vm_i>=vm_min:
                        mask[i] = True
                r = eval_config(mask, tp, mh)
                if r:
                    r["cfg"] = f"GK>={gk_min}+VM>={vm_min} tp{tp} mh{mh}"
                    results_gke4.append(r)

print_top("GK Expansion + Volume", results_gke4)


# ===== V11-E S Baseline =====
print("\n" + "="*70)
print("  V11-E S Baseline")
print("="*70)
mask_v11e = (df["gk_pct"] < 30).values & df["brk_dn_15"].values
r = eval_config(mask_v11e, 2.0, 7, sn_pct=4.0, cd=8, mcap=-150)
if r:
    print(f"  V11-E S: IS {r['is_n']}t ${r['is_pnl']:+.0f} WR {r['is_wr']:.1f}% | OOS {r['oos_n']}t ${r['oos_pnl']:+.0f} WR {r['oos_wr']:.1f}% | PM {r['pm']}/{r['tot_m']} WM ${r['worst']:+.0f}")


# ===== Best overall + monthly =====
all_results = results_gke + results_gke2 + results_gke3 + results_gke4
all_results.sort(key=lambda x: x["oos_pnl"], reverse=True)

print(f"\n{'='*70}")
if all_results and all_results[0]["oos_pnl"] > 0:
    best = all_results[0]
    print(f"  BEST GK EXPANSION: {best['cfg']}")
    print(f"  IS: {best['is_n']}t ${best['is_pnl']:+.0f} WR {best['is_wr']:.1f}%")
    print(f"  OOS: {best['oos_n']}t ${best['oos_pnl']:+.0f} WR {best['oos_wr']:.1f}%")
    print(f"  PM: {best['pm']}/{best['tot_m']}  Worst: ${best['worst']:+.0f}")

    # If best is good enough, show monthly
    if best['oos_pnl'] >= 500:
        # Re-build mask for monthly detail
        cfg = best['cfg']
        gk_min = int(cfg.split("GK>=")[1].split(" ")[0].split("+")[0])
        mask = np.array([False] * len(df))
        if "+EMA>=" in cfg:
            ed_min = float(cfg.split("EMA>=")[1].split(" ")[0])
            for i in range(len(df)):
                g = gk_vals[i]; e = ed_vals[i]
                if g==g and e==e and g>=gk_min and e>=ed_min:
                    mask[i] = True
        elif "+bearish" in cfg or "+close_low" in cfg:
            btype = "bearish" if "+bearish" in cfg else "close_low"
            bv = bear_vals if btype == "bearish" else cl_vals
            for i in range(len(df)):
                g = gk_vals[i]
                if g==g and g>=gk_min and bv[i]:
                    mask[i] = True
        elif "+VM>=" in cfg:
            vm_min = float(cfg.split("VM>=")[1].split(" ")[0])
            for i in range(len(df)):
                g = gk_vals[i]; vm_i = vm_vals[i]
                if g==g and vm_i==vm_i and g>=gk_min and vm_i>=vm_min:
                    mask[i] = True
        else:
            for i in range(len(df)):
                g = gk_vals[i]
                if g==g and g>=gk_min:
                    mask[i] = True

        oos_t = run_short_generic(mask, best["tp_pct"], best["mh"], 4.0, 8, -150, IS_END, TOTAL)
        oos_m = pd.DataFrame(oos_t).groupby("month")["pnl"].sum()

        v11e_l = {"2025-04":117,"2025-05":329,"2025-06":19,"2025-07":248,"2025-08":115,
                  "2025-09":62,"2025-10":-111,"2025-11":139,"2025-12":105,
                  "2026-01":-24,"2026-02":355,"2026-03":117,"2026-04":166}
        v11e_s = {"2025-04":106,"2025-05":174,"2025-06":0,"2025-07":-139,"2025-08":4,
                  "2025-09":100,"2025-10":296,"2025-11":13,"2025-12":94,
                  "2026-01":108,"2026-02":274,"2026-03":296,"2026-04":-175}

        months = sorted(set(oos_m.index) | set(v11e_l.keys()))
        print(f"\n{'Month':>10} {'L(V11E)':>8} {'S(new)':>8} {'L+Snew':>8} | {'S(V11E)':>8} {'L+Sv11':>8} | {'Delta':>7}")
        t_new = t_old = 0
        for m in months:
            lp = v11e_l.get(m, 0)
            sp_new = oos_m.get(m, 0); sp_old = v11e_s.get(m, 0)
            ls_new = lp + sp_new; ls_old = lp + sp_old
            t_new += ls_new; t_old += ls_old
            print(f"  {m:>10} {lp:>+8.0f} {sp_new:>+8.0f} {ls_new:>+8.0f} | {sp_old:>+8.0f} {ls_old:>+8.0f} | {ls_new-ls_old:>+7.0f}")
        print(f"  {'TOTAL':>10} {'':>8} {'':>8} {t_new:>+8.0f} | {'':>8} {t_old:>+8.0f} | {t_new-t_old:>+7.0f}")
else:
    print(f"  NO OOS-positive candidate found")
print(f"{'='*70}")
