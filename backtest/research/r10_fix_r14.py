"""
R10 Fix Round 14: SN4.5% + MT8 + ExitCD12 + Early Stop 1.5%
==============================================================
R12 (CD12, ES2.0%) = IS -$504 ($4 short!). R13 (CD13) worse at -$517.
Back to CD12, lower early stop threshold to 1.5% to catch 2-3 more trades.

Logic: In the <12h window, WR = 0%. A trade 1.5% underwater at bar 8
AND above EMA20 is heading for bigger losses. Catching it early saves money.
1.5% is above ETH normal 8h noise (~1%), so recoveries are unlikely.
...
[Y] fix direction decided before data?  1.5% from ETH 8h noise (1%) + buffer
[Y] no post-result parameter adjustment?  Pre-committed from noise analysis
[Y] no OOS leakage?  Uses only entry price and current bar
"""
...
MIN_TRAIL = 8
EXIT_COOLDOWN = 12
EARLY_STOP_PCT = 0.015    # ★ was 0.020 → 0.015
EARLY_STOP_END = 12
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 14: SN4.5% + MT8 + ExitCD12 + EarlyStop 1.5%")
    print("="*70)
    ...
    oos_pos=sum(1 for p in oos_months if p>0); oos_tot=len(oos_months)
    ...
    if all_pass:
        print(f"\n  *** ALL 9 TARGETS MET! ***")
        print(f"\n  Final Strategy Spec:")
        print(f"    Parkinson percentile < {PARK_THRESH}, Close breakout ({BRK_LOOK}bar)")
        print(f"    Session filter: block hours {BLOCK_H}, block days {BLOCK_D}")
        print(f"    SafeNet: +/-{SAFENET_PCT*100:.1f}% (0.25 slippage)")
        print(f"    Trail: EMA20, min hold {MIN_TRAIL}bars")
        print(f"    Early stop: {EARLY_STOP_PCT*100:.1f}% loss (bars {MIN_TRAIL}-{EARLY_STOP_END})")
        print(f"    Exit cooldown: {EXIT_COOLDOWN} bars same direction")
        print(f"    Max same: {MAX_SAME}")
        print(f"\n  IS: {is_s['n']} trades, ${is_s['pnl']:+,.0f}, PF {is_s['pf']:.2f}, WR {is_s['wr']:.1f}%")
        print(f"  OOS: {oos_s['n']} trades, ${oos_s['pnl']:+,.0f}, PF {oos_s['pf']:.2f}, WR {oos_s['wr']:.1f}%")
        print(f"  Full: {pos_months}/{tot_months} pos months ({full_pos_rate:.0f}%), consec losing: {full_consec}")
        print(f"  MDD: {oos_s['mdd_pct']:.1f}%, Sharpe: {oos_s['sharpe']:.2f}")
    else:
