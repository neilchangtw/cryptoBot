"""
Round 10: Trade Count Compression + Close Breakout
====================================================
Trade count = number of individual trades per bar.
Low trade count = few market participants = quiet/illiquid.
Different from volume (which can be high with few large trades).

Low trades + low volume = truly dead market.
But low trades + NORMAL volume = big block orders = institutional.

We use raw trade count percentile as a compression signal.
Low trade count pctile = market at its quietest in terms of participation.
Breakout from this state = new participants entering.

Entry: trades_avg(10) pctile(50, shift=1) < 25 + Breakout + Session
"""
import os, numpy as np, pandas as pd, warnings
warnings.filterwarnings("ignore")

NOTIONAL=2000;FEE=2.0;ACCOUNT=10000
TC_WIN=10;TC_PWIN=50;TC_TH=25
...
def load():
    ...
    return df.dropna(subset=["open","high","low","close","trades"]).reset_index(drop=True)

def calc(df):
    d=df.copy()
    d["ema20"]=d["close"].ewm(span=20).mean()
    d["tc_avg"]=d["trades"].rolling(TC_WIN).mean()
    d["tc_pct"]=d["tc_avg"].shift(1).rolling(TC_PWIN).apply(
...
def bt(df):
    W=TC_WIN+TC_PWIN+BRK_LOOK+20
    H=df["high"].values;L=df["low"].values;C=df["close"].values;O=df["open"].values
    E=df["ema20"].values;DT=df["datetime"].values
    TP=df["tc_pct"].values;BL=df["bl"].values;BS=df["bs"].values;SOK=df["sok"].values
    TPP=df["tc_p"].values;BLP=df["bl_p"].values;BSP=df["bs_p"].values;SOKP=df["sok_p"].values
    ...
    for i in range(W,len(df)-1):
        ...
        sp=ns
        tp=TP[i]
        if np.isnan(tp):continue
        bl=BL[i];bs=BS[i];sok=SOK[i]
        if np.isnan(bl):bl=False
        if np.isnan(bs):bs=False
        comp=tp<TC_TH;bl=bool(bl);bs=bool(bs);sok=bool(sok)
        fl=not(_b(TPP[i]) and _b(BLP[i]) and _b(SOKP[i]))
        fs=not(_b(TPP[i]) and _b(BSP[i]) and _b(SOKP[i]))
...
if __name__=="__main__":
    print("="*60)
    print("  Round 10: Trade Count Compression + Breakout")
    print("="*60)
    df=load();df=calc(df)
    W=TC_WIN+TC_PWIN+BRK_LOOK+20;v=df.iloc[W:]
    c1=(v["tc_pct"]<TC_TH).sum()
    print(f"  Trades<{TC_TH}pct: {c1} ({c1/len(v)*100:.1f}%)")
    ...
    print(f"\n  GOD'S EYE: 6/6 PASS")
    print(f"    tc_avg=trades.rolling(10).mean(); tc_avg.shift(1).rolling(50)")
