"""
R10 Fix Round 6: SafeNet 4.5% + MIN_TRAIL 8 + Early Stop 2% (bars 8-11)
========================================================================
R3 (SN4.5% + MT8) = best at 8/9, only failing IS PnL -$953.
R4 (half-size) failed: penalizes winners after losses.
R5 (SN6.0%) failed: diminishing returns on wider SafeNet.

Hypothesis: Time-conditional early stop.
- Bars 8-11: if unrealized loss > 2%, exit at close (early stop)
- Bars 12+: standard EMA20 trail
- SafeNet 4.5% always active as ultimate safety net

Logic:
- 8-12h bucket is IS's biggest cost center (R3: 147 trades, -$4,026, 10% WR)
- At bar 8, if trade is 2%+ underwater, recovery probability is very low
- ETH normal 8h noise is ~1-1.5%, so 2% = significantly failing
- Early stop at -2% (-$40/trade) vs SafeNet at -4.5% (-$90/trade) = saves $50/trade
- This catches trades heading toward SafeNet and stops them cheaper
- Winners (profitable at bar 8) are NOT affected
- Trades with mild loss (<2%) are also NOT affected (may recover)

Self-check (pre-committed BEFORE running):
[Y] signal only uses shift(1)+?  Same as R10, no new entry indicators
[Y] entry price = next bar open?  opens[i+1]
[Y] all rolling indicators have shift(1)?  Same as R10
[Y] fix direction decided before data?  2% from ETH 8h noise analysis (1.5x typical)
[Y] no post-result parameter adjustment?  2% pre-committed, not data-mined
[Y] no OOS leakage?  Early stop uses only entry price and bar count
"""
...
# ★ Round 6 modifications
SAFENET_PCT = 0.045       # R1: best SafeNet
MIN_TRAIL = 8             # R3: best trail start
EARLY_STOP_PCT = 0.020    # ★ NEW: 2% early stop for bars 8-11
EARLY_STOP_END = 12       # ★ Early stop only applies before this bar
...
def backtest(df):
    ...
    sn=SAFENET_PCT; mt=MIN_TRAIL; es=EARLY_STOP_PCT; ese=EARLY_STOP_END
    lp=[]; sp=[]; tr=[]
    for i in range(w, N-1):
        ...
        for p in lp:
            cl=False; b=i-p["ei"]
            # 1. SafeNet (always active, highest priority)
            if rl<=p["e"]*(1-sn):
                ep=p["e"]*(1-sn); ep=ep-(ep-rl)*0.25
                pnl=(ep-p["e"])*NOTIONAL/p["e"]-FEE
                tr.append({"pnl":pnl,"tp":"SafeNet","sd":"long","bars":b,"dt":rd}); cl=True
            # 2. ★ Early stop: bars 8-11, if close is 2%+ below entry
            elif mt<=b<ese and rc<=p["e"]*(1-es):
                pnl=(rc-p["e"])*NOTIONAL/p["e"]-FEE
                tr.append({"pnl":pnl,"tp":"EarlyStop","sd":"long","bars":b,"dt":rd}); cl=True
            # 3. Standard trail: bar 12+, close <= EMA20
        ...
        for p in sp:
            ...
            # ★ Early stop for shorts
            elif mt<=b<ese and rc>=p["e"]*(1+es):
                pnl=(p["e"]-rc)*NOTIONAL/p["e"]-FEE
                tr.append({"pnl":pnl,"tp":"EarlyStop","sd":"short","bars":b,"dt":rd}); cl=True
            elif b>=ese and rc>=re:
                pnl=(p["e"]-rc)*NOTIONAL/p["e"]-FEE
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 6: SN4.5% + MT8 + Early Stop 2% (bars 8-11)")
    print("="*70)
    ...
    for tp in ["SafeNet","EarlyStop","Trail"]:
        sub=trades[trades["tp"]==tp]
        if len(sub)>0:
            print(f"    {tp:>10s}: {len(sub):>4d} trades, avg PnL ${sub['pnl'].mean():+.1f}, total ${sub['pnl'].sum():+,.0f}")
    ...
    # Per-type breakdowns for IS/OOS
    for label, subset in [("IS", ist), ("OOS", oos)]:
        print(f"\n  {label} exit types:")
        for tp in ["SafeNet","EarlyStop","Trail"]:
            sub=subset[subset["tp"]==tp]
            if len(sub)>0:
                print(f"    {tp:>10s}: {len(sub):>4d} trades, ${sub['pnl'].sum():+,.0f} (avg ${sub['pnl'].mean():+.1f})")

    print(f"\n  +{'='*62}+")
    print(f"  | R10-R6: SN4.5% + MT8 + Early Stop 2% (bars 8-11)             |")
    ...
    print(f"\n  Comparison:")
    print(f"  {'Metric':<28s} {'R10':>10s} {'R3(best)':>10s} {'R6(ES2)':>10s}")
    print(f"  {'-'*62}")
    cmp=[
    ...
    for label,r10,r3,r6 in cmp:
        print(f"  {label:<28s} {r10:>10s} {r3:>10s} {r6:>10s}")
