"""
Round 10: Parkinson Compression + Close Breakout + Session (Lean 3-factor)
==========================================================================
Core: Simplest viable combination. Parkinson compression detects volatility
squeeze, close breakout confirms direction, session filter removes
low-liquidity noise hours/days. No volume, no ADX, no cross-asset.

Entry (3 conditions):
  1. Parkinson Vol Ratio pctile(100, shift=1) < 30 (compression)
  2. Close Breakout: close[T-1] > max(close[T-2..T-11]) (long) / < min() (short)
  3. Session: NOT block hours {0,1,2,12} UTC+8, NOT block days {Mon,Sat,Sun}
  4. Fresh: not all conditions met at T-2
...
def load_data():
    ...
    print("Need to fetch data"); sys.exit(1)
...
def run_backtest(df):
    warmup = PARK_PCTILE_WIN + PARK_LONG + BREAKOUT_LOOKBACK + 10
    ...
    for i in range(warmup, len(df) - 1):
        ...
        park = pp[i]; bl = bls[i]; bs = bss[i]; so = sess[i]
        if np.isnan(park): continue
        ...
        park_p = pp_prev[i]; bl_p = bls_prev[i]; bs_p = bss_prev[i]; so_p = sess_prev[i]
        if not np.isnan(park_p):
            p_comp = park_p < PARK_THRESH
            p_bl = bool(bl_p) if not np.isnan(bl_p) else False
            p_bs = bool(bs_p) if not np.isnan(bs_p) else False
            p_so = bool(so_p)
        else:
            p_comp = p_bl = p_bs = p_so = False

        fresh_long = not (p_comp and p_bl and p_so)
        fresh_short = not (p_comp and p_bs and p_so)

        long_signal = compressed and long_brk and session_ok and fresh_long
        short_signal = compressed and short_brk and session_ok and fresh_short
...
def gods_eye_check():
    ...
    checks = [
        ("1. All signal calcs use shift(1)+ data only?",
         ...
         "         session uses datetime (structural, not data-derived)"),
        ("2. Entry price = next bar open?","YES: entry = nxt_open = opens[i+1]"),
        ("3. All rolling indicators have shift(1)?",
         "YES: Parkinson shift(1).rolling(100); Breakout close.shift(1) vs close.shift(2).rolling(9)"),
        ("4. Params decided before seeing any data?",
         "YES: Parkinson 5/20/30 from vol theory; breakout 10 from structure;\n"
         "         Session from well-documented crypto calendar effects"),
        ("5. No post-OOS parameter adjustment?","YES: Single run"),
        ("6. Freshness check prevents stale re-entry?","YES: Checks comp+brk+session at prev bar")
    ]
    ap=True
    for q,a in checks:
...
if __name__ == "__main__":
    print("="*80)
    print("  Round 10: Parkinson + Close Breakout + Session (Lean 3-factor)")
    print("="*80)
