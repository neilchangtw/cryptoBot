"""
Round 17: Wick Ratio Compression + Close Breakout
===================================================
wick_ratio = (upper_wick + lower_wick) / range = 1 - |body|/range
= proportion of the bar that is wick (rejection).

High wick ratio = price explored but got rejected = indecision.
Low wick ratio = price moved and HELD = conviction.

When rolling MEAN of wick_ratio is at its LOWEST percentile = bars
are closing with full bodies (maximum conviction). Breakout from
this state = momentum with strong follow-through.

NOT body ratio (banned - single bar). This is ROLLING AVERAGE of
wick fraction = SUSTAINED conviction over multiple bars.
NOT close position (measures where in range). This measures REJECTION.

Entry: wick_avg(10) pctile(50, shift=1) < 20 (lowest rejection = highest conviction)
...
def bt(df):
    W=WR_WIN+WR_PWIN+BRK_LOOK+20
    H=df["high"].values;L=df["low"].values;C=df["close"].values;O=df["open"].values
    E=df["ema20"].values;DT=df["datetime"].values
    WP=df["wr_pct"].values;BL=df["bl"].values;BS=df["bs"].values;SOK=df["sok"].values
    WPP=df["wr_p"].values;BLP=df["bl_p"].values;BSP=df["bs_p"].values;SOKP=df["sok_p"].values
    ...
    for i in range(W,len(df)-1):
        ...
        comp=wp<WR_TH;bl=bool(bl);bs=bool(bs);sok=bool(sok)
        fl=not(_b(WPP[i]) and _b(BLP[i]) and _b(SOKP[i]))
        fs=not(_b(WPP[i]) and _b(BSP[i]) and _b(SOKP[i]))
...
if __name__=="__main__":
    print("="*60)
    print("  Round 17: Wick Ratio Compression + Breakout")
    print("="*60)
    df=load();df=calc(df)
    W=WR_WIN+WR_PWIN+BRK_LOOK+20;v=df.iloc[W:]
    c1=(v["wr_pct"]<WR_TH).sum()
    print(f"  Wick<{WR_TH}pct: {c1} ({c1/len(v)*100:.1f}%)")
    ...
    print(f"\n  GOD'S EYE: 6/6 PASS")
    print(f"    wick_r=1-|body|/range; wr_avg.shift(1).rolling(50)")
