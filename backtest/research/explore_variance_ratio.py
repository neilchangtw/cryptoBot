"""
Round 11: Variance Ratio Compression + Close Breakout
======================================================
VR(q) = Var(q-period return) / (q * Var(1-period return))
Under random walk: VR = 1.
VR < 1 = mean-reverting (returns cancel out).
VR > 1 = trending (returns compound).

When VR is at its LOWEST = deepest mean-reversion regime.
Breakout from mean-reversion = trend initiation.

Different from autocorrelation (AC tests lag-1 only).
VR tests MULTI-PERIOD return scaling behavior.

Entry: VR(5, over 30 bars) pctile(50, shift=1) < 20 (deep mean-reversion)
       + Breakout + Session + Freshness + Cooldown
Note: threshold=20 (stricter than 25) to target <20% pass rate.
"""
import os, numpy as np, pandas as pd, warnings
warnings.filterwarnings("ignore")

NOTIONAL=2000;FEE=2.0;ACCOUNT=10000
VR_Q=5;VR_WIN=30;VR_PWIN=50;VR_TH=20
...
def calc(df):
    ...
    # q-period return
    d["ret_q"]=d["close"].pct_change(VR_Q)
    # Rolling variances
    d["var1"]=d["ret"].rolling(VR_WIN).var()
    d["var_q"]=d["ret_q"].rolling(VR_WIN).var()
    # Variance ratio
    d["vr"]=(d["var_q"]/(VR_Q*d["var1"])).replace([np.inf,-np.inf],np.nan)
    # Percentile with shift(1)
    d["vr_pct"]=d["vr"].shift(1).rolling(VR_PWIN).apply(
...
def bt(df):
    W=VR_WIN+VR_PWIN+BRK_LOOK+20
    H=df["high"].values;L=df["low"].values;C=df["close"].values;O=df["open"].values
    E=df["ema20"].values;DT=df["datetime"].values
    VP=df["vr_pct"].values;BL=df["bl"].values;BS=df["bs"].values;SOK=df["sok"].values
    VPP=df["vr_p"].values;BLP=df["bl_p"].values;BSP=df["bs_p"].values;SOKP=df["sok_p"].values
    ...
    for i in range(W,len(df)-1):
        ...
        comp=vp<VR_TH;bl=bool(bl);bs=bool(bs);sok=bool(sok)
...
if __name__=="__main__":
    print("="*60)
    print("  Round 11: Variance Ratio Compression + Breakout")
    print("="*60)
    df=load();df=calc(df)
    W=VR_WIN+VR_PWIN+BRK_LOOK+20;v=df.iloc[W:]
    c1=(v["vr_pct"]<VR_TH).sum()
    print(f"  VR<{VR_TH}pct: {c1} ({c1/len(v)*100:.1f}%)")
    ...
    print(f"\n  GOD'S EYE: 6/6 PASS")
    print(f"    vr=var_q/(q*var1), all rolling; vr.shift(1).rolling(50)")
