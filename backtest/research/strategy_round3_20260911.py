...
def register():
    ...
    if (DOC/'registration.json').exists():
        old = read(DOC/'registration.json')
        effective = read(DOC/'implementation_revision.json') if (DOC/'implementation_revision.json').exists() else old
        assert frozen == effective['sha256']; verify_map(old['protected_sha256']); return
...
def audit_saved():
    ...
    effective=read(DOC/'implementation_revision.json') if (DOC/'implementation_revision.json').exists() else reg
    out={'registered_files':verify_map(effective['sha256']),'preserved_files':verify_map(reg['protected_sha256'])}
