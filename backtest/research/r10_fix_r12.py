"""
R10 Fix Round 12: SN4.5% + MT8 + Exit Cooldown 12 + Early Stop 2%
===================================================================
R11 = 8/9, IS PnL -$558 (need > -$500). Only $58 short!
R7's 2% early stop improved IS by $54 (from -$953 to -$899).
These two mechanisms work on DIFFERENT axes:
  - Exit cooldown: reduces trade COUNT (removes worst re-entries)
  - Early stop: reduces LOSS PER TRADE in 8-12h window
They should stack additively.
...
[Y] fix direction decided before data?  Both components pre-validated in R7/R11
[Y] no post-result parameter adjustment?  Combining two pre-validated improvements
[Y] no OOS leakage?  Both use only past trade data
"""
...
MIN_TRAIL = 8
EXIT_COOLDOWN = 12
EARLY_STOP_PCT = 0.020    # 2% loss stop for bars 8-11
EARLY_STOP_END = 12       # switch to EMA20 trail at bar 12
    ...
    for i in range(w, N-1):
        ...
        for p in lp:
            ...
            if rl<=p["e"]*(1-sn):
                ...
                last_long_exit = i
            # Bars 8-11: EMA20 trail OR 2% early stop
            elif mt<=b<ese:
                trail_hit = rc<=re
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 12: SN4.5% + MT8 + ExitCD12 + EarlyStop 2%")
    print("="*70)
    ...
    print(f"\n  +{'='*62}+")
    print(f"  | R10-R12: SN4.5% + MT8 + ExitCD12 + EarlyStop 2%              |")
