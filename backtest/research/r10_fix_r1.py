"""
R10 Fix Round 1: SafeNet 4.5% + Compression Duration Cap 20h
=============================================================
Hypothesis:
1. SafeNet 3.5% -> 4.5%: ETH daily range 3-4%, current SN within noise.
   Widening gives breakouts room to breathe through intra-day oscillation.
2. Compression duration cap 20h: prolonged compression (>20h) = dead market,
   not genuine pre-trend compression. Filter reduces false breakouts in chop.

Self-check (pre-committed BEFORE running):
[Y] signal only uses shift(1)+?  park_pctile=shift(1), breakout=shift(1)+shift(2),
    session=structural, comp_dur=derived from shifted pp
[Y] entry price = next bar open?  opens[i+1]
[Y] all rolling indicators have shift(1)?  pp=shift(1).rolling(), brk=shift(1)
[Y] fix direction decided before data?  SafeNet from validation finding,
    duration from market structure logic
[Y] no post-result parameter adjustment?  4.5% from ETH vol logic, 20h from
    typical consolidation duration
[Y] no OOS leakage?  all indicators in rolling windows, duration from shifted pp
"""
import os, sys, pandas as pd, numpy as np
from datetime import datetime, timedelta
import warnings; warnings.filterwarnings("ignore")

# === Constants (same as R10 except noted) ================================
...
SAFENET_PCT = 0.045       # was 0.035
MAX_COMP_DUR = 20         # NEW: max consecutive bars in compression
...
def compute(df):
    ...
    # ★ NEW: Compression duration (consecutive bars with pp < PARK_THRESH)
    pp_vals = d["pp"].values
    cd = np.zeros(len(d), dtype=int)
    for i in range(1, len(d)):
        if not np.isnan(pp_vals[i]) and pp_vals[i] < PARK_THRESH:
            cd[i] = cd[i-1] + 1
        else:
            cd[i] = 0
    d["cd"] = cd
...
def backtest(df):
    ...
    CD=df["cd"].values  # ★ compression duration
    ...
    for i in range(w, N-1):
        ...
        if co and blo and s and fl and dur_ok and len(lp)<MAX_SAME:
            lp.append({"e":no,"ei":i})
        if co and bso and s and fs and dur_ok and len(sp)<MAX_SAME:
            sp.append({"e":no,"ei":i})
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 1: SafeNet 4.5% + Compression Duration Cap 20h")
    print("="*70)
    ...
    # Diagnostic: compression duration stats
    w=PARK_WIN+PARK_LONG+20; v=df.iloc[w:]
    comp_bars=(v["pp"]<PARK_THRESH).sum()
    comp_pct=comp_bars/len(v)*100
    dur_exceed=(v["cd"]>MAX_COMP_DUR).sum()
    dur_exceed_pct=dur_exceed/comp_bars*100 if comp_bars>0 else 0
    print(f"\n  Diagnostic:")
    print(f"    pp<30 bars: {comp_bars} ({comp_pct:.1f}%)")
    print(f"    duration>20 bars: {dur_exceed} ({dur_exceed_pct:.1f}% of compressed)")
    print(f"    Expected trade reduction: ~{dur_exceed_pct:.0f}% of compressed signals filtered")
    ...
    print(f"  | Max consecutive losing (OOS): {oos_consec}{'':>27s}|")
    ...
    print(f"  | Max consecutive losing (full): {full_consec}{'':>26s}|")
    print(f"  +{'='*62}+")

    # Monthly detail
    print(f"\n  Monthly PnL:")
    print(f"  {'Month':<10s} {'N':>4s} {'PnL':>10s} {'Cum':>10s} {'':>4s}")
    print(f"  {'-'*35}")
    cum=0
    ...
    # Comparison with R10 baseline
    print(f"\n  Comparison vs R10 baseline:")
    print(f"  {'Metric':<22s} {'R10':>10s} {'R10-R1':>10s} {'Delta':>10s}")
    print(f"  {'-'*55}")
    cmp=[
    ...
    for label,r10,r1,delta in cmp:
        print(f"  {label:<22s} {r10:>10s} {r1:>10s} {delta:>10s}")
    ...
    t8=full_consec<=3
    t9=True  # God's eye self-check passed above
