"""
R10 Fix Round 8: SN4.5% + MT8 + PARK_THRESH 35 + Combined Exit
================================================================
R7 = 8/9, IS PnL -$899 (need > -$500). $399 gap still large.
Exit tweaks (R5-R7) only gave marginal IS improvement.

Hypothesis: Increase PARK_THRESH from 30 to 35.
- Validation dim4 showed PT=33-36 outperforms PT=30 overall
- More trades = more winners + smoother monthly PnL
- Additional trades (at PT 30-35) have positive edge per validation
- Combined with all R7 improvements (SN4.5%, MT8, 2% early stop)

Self-check:
[Y] signal only uses shift(1)+?  Same indicators, only threshold changed
[Y] entry price = next bar open?  opens[i+1]
[Y] all rolling indicators have shift(1)?  Same as R10
[Y] fix direction decided before data?  PT=35 from validation finding (33-36 band)
[Y] no post-result parameter adjustment?  35 = middle of validated 33-36 range
[Y] no OOS leakage?  Parkinson percentile is rolling within-sample
"""
...
# ★ Round 8 modifications
SAFENET_PCT = 0.045
MIN_TRAIL = 8
EARLY_STOP_PCT = 0.020
EARLY_STOP_END = 12
PARK_THRESH = 35          # ★ was 30 → 35
...
def backtest(df):
    ...
    for i in range(w, N-1):
        ...
        co=pp_v<pt  # ★ using variable threshold
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 8: SN4.5% + MT8 + PT35 + Combined Exit")
    print("="*70)
    ...
    for tp in ["SafeNet","EarlyStop","Trail"]:
        ...
        if len(sub)>0:
            print(f"    {tp:>10s}: {len(sub):>4d} trades, avg ${sub['pnl'].mean():+.1f}, total ${sub['pnl'].sum():+,.0f}")
    ...
    print(f"\n  +{'='*62}+")
    print(f"  | R10-R8: SN4.5% + MT8 + PT35 + Combined Exit                  |")
    ...
    print(f"  | OOS pos months: {oos_pos}/{oos_tot}  consec losing: {oos_consec}{'':>23s}|")
    ...
    print(f"\n  Comparison:")
    print(f"  {'Metric':<28s} {'R3':>10s} {'R7':>10s} {'R8':>10s}")
    print(f"  {'-'*62}")
    cmp=[
        ("OOS PnL","$+5,347","$+5,202",f"${oos_s['pnl']:>+,.0f}"),
        ("OOS PF","2.25","2.20",f"{oos_s['pf']:.2f}"),
        ("IS PnL","$-953","$-899",f"${is_s['pnl']:>+,.0f}"),
        ("Full pos months","14/25 (56%)","14/25 (56%)",f"{pos_months}/{tot_months} ({full_pos_rate:.0f}%)"),
        ("Max consec losing","3","3",f"{full_consec}"),
        ("OOS max month%","32.9%","33.8%",f"{oos_max_month_pct:.1f}%"),
    ]
    for label,r3,r7,r8 in cmp:
        print(f"  {label:<28s} {r3:>10s} {r7:>10s} {r8:>10s}")
