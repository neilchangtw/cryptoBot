"""
R10 Fix Round 7: SafeNet 4.5% + MT8 + Combined Exit (EMA20 trail OR 2% stop)
=============================================================================
R6 FAILED because early stop REPLACED EMA20 trail at bars 8-11.
Fix: bars 8-11 use BOTH checks (OR logic), not just the 2% stop.

Exit logic:
- SafeNet 4.5% (always active)
- Bars 8-11: exit if (close <= EMA20) OR (unrealized loss > 2%)
- Bars 12+: exit if close <= EMA20 (standard trail)

This is R3 + supplementary 2% stop for bars 8-11 only.
The 2% stop catches Case B trades: 2%+ underwater but ABOVE EMA20
(false breakouts during declining trend where EMA20 is even lower).
...
[Y] fix direction decided before data?  2% from ETH noise; combining exits, not new indicator
[Y] no post-result parameter adjustment?  Same 2% as R6 (pre-committed)
[Y] no OOS leakage?  Uses entry price, bar count, and existing EMA20
"""
...
SAFENET_PCT = 0.045
MIN_TRAIL = 8
EARLY_STOP_PCT = 0.020    # 2% loss threshold
EARLY_STOP_END = 12       # early stop only for bars < 12
...
def backtest(df):
    ...
    for i in range(w, N-1):
        ...
        for p in lp:
            ...
            # 2. Bars 8-11: EMA20 trail OR 2% early stop
            elif mt<=b<ese:
                trail_hit = rc<=re
            ...
            # 3. Bars 12+: standard EMA20 trail
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 7: SN4.5% + MT8 + Combined Exit (Trail OR 2% stop)")
    print("="*70)
    ...
    print(f"\n  +{'='*62}+")
    print(f"  | R10-R7: SN4.5% + MT8 + Combined (Trail OR 2%) bars 8-11      |")
    ...
    print(f"\n  Comparison:")
    print(f"  {'Metric':<28s} {'R10':>10s} {'R3(best)':>10s} {'R7':>10s}")
    print(f"  {'-'*62}")
    cmp=[
    ...
    for label,r10,r3,r7 in cmp:
        print(f"  {label:<28s} {r10:>10s} {r3:>10s} {r7:>10s}")
