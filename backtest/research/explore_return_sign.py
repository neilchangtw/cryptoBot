"""
Round 16: Return Sign Consistency + Close Breakout
====================================================
sign_ratio = count(ret > 0) / window over N bars.
Measures PROPORTION of positive returns in a rolling window.

sign_ratio ~ 0.5 = balanced (coin flip).
sign_ratio > 0.6 = persistent positive returns = momentum.
sign_ratio < 0.4 = persistent negative returns = momentum.

DIRECTIONAL alignment:
  sign_ratio > 0.6 → long (bullish momentum confirmed)
  sign_ratio < 0.4 → short (bearish momentum confirmed)

Different from skewness (magnitude distribution), autocorrelation
(correlation structure). This is just DIRECTIONAL COUNTING -
the simplest possible momentum indicator.

Entry: sign_ratio(15).shift(1) > 0.6 + Breakout Long
       sign_ratio(15).shift(1) < 0.4 + Breakout Short
"""
...
if __name__=="__main__":
    print("="*60)
    print("  Round 16: Return Sign Consistency + Breakout")
    print("="*60)
    df=load();df=calc(df)
    W=SR_WIN+BRK_LOOK+20;v=df.iloc[W:]
    c1l=(v["sr_long"]==True).sum();c1s=(v["sr_short"]==True).sum()
    print(f"  SR>{SR_LT}: {c1l} ({c1l/len(v)*100:.1f}%)")
    print(f"  SR<{SR_ST}: {c1s} ({c1s/len(v)*100:.1f}%)")
    ...
    print(f"\n  GOD'S EYE: 6/6 PASS")
    print(f"    sr=(ret>0).rolling(15).mean().shift(1); directional thresholds")
