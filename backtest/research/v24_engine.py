"""
V24 Engine — V14+R 擴充版，支援 per-side notional / fee 參數
目的：方向 B 降槓桿重校準測試

相對 v23_overlay_engine 的差異：
  - NOTIONAL_L / NOTIONAL_S 可獨立設定（原本統一 $4000）
  - FEE_L / FEE_S 可獨立設定（原本統一 $4）
  - CB_DAILY / CB_L_MONTH / CB_S_MONTH 按 notional 比例縮放（保持相同 % 風險門檻）
  - 其他 V14 規則 100% 沿用，overlay block_bars / size_scale 沿用

預設值與 V23 引擎一致（NOTIONAL=$4000/FEE=$4），反向相容。
"""
import os
import numpy as np
import pandas as pd
...
def run_v14_overlay(o, h, l, c, hours, dows, mks, dks, start, end,
                    ...
                    notional_L=4000, notional_S=4000, fee_L=4, fee_S=4,
                    cb_daily=None, cb_l_month=None, cb_s_month=None,
                    cb_consec=4, cb_consec_cd=24):
    """
    notional_L / notional_S: per-side 名目金額
    fee_L / fee_S: per-side 單筆費用
    cb_daily / cb_l_month / cb_s_month: 若為 None，按 notional 比例縮放預設值
      預設值（$4000 notional）: daily -$200, L_month -$75, S_month -$150
    """
    # Scale CB thresholds proportionally to notional (default V14 values at $4000)
    if cb_daily is None:
        # Use max of L/S notional for daily (account-level)
        cb_daily = -200 * (max(notional_L, notional_S) / 4000)
    if cb_l_month is None:
        cb_l_month = -75 * (notional_L / 4000)
    if cb_s_month is None:
        cb_s_month = -150 * (notional_S / 4000)
    ...
    for i in range(start, end):
        ...
        if lp_active:
            ...
            if ex_p>0:
                pnl_pct=(ex_p-ep)/ep
                pnl = pnl_pct*notional_L*lp_scale - fee_L*lp_scale
        ...
        if sp_active:
            ...
            if ex_p>0:
                pnl_pct=(ep-ex_p)/ep
                pnl = pnl_pct*notional_S*sp_scale - fee_S*sp_scale
