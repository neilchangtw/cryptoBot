...
def register():
    ...
    if (DOC/'registration.json').exists():
        ...
        revisions=sorted(DOC.glob('implementation_revision*.json'))
        effective=read(revisions[-1]) if revisions else reg
        assert inputs==effective['sha256']; prior_audit.verify_map(reg['protected_sha256']); return
...
class Study:
    ...
    def run(self,name='base',slip=0,cut=None,save=True):
        ...
        # An early physical prefix may contain an open position and zero completed trades.
        # Read column names only; never create a synthetic close or use future trade values.
        f=previous.review.normalize(pd.DataFrame(raw)) if raw else pd.read_csv(OLD/'base_0bp_trades.csv',nrows=0,parse_dates=['entry_dt','exit_dt'])
    ...
    def noop(self):
        ...
        for slip in [0,2,5]:
            ...
            for suffix,cur,dates in [('trades',f,['entry_dt','exit_dt']),('equity',eq,['time']),('gates',ev,['decision_ts']),('states',st,None)]:
                prev=pd.read_csv(OLD/f'base_{slip}bp_{suffix}.csv',parse_dates=dates,keep_default_na=False) if suffix=='states' else pd.read_csv(OLD/f'base_{slip}bp_{suffix}.csv',parse_dates=dates)
