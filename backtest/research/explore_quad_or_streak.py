"""
Round 21: Quad OR with Direction Streak (GK+Skew+RetSign+Streak)
================================================================
R18 Triple OR = $9,863 OOS (best). R19 Quad+Wick = $9,423 (WK negative).
R20 Triple OR TH=0.55 = $9,279 (diluted).

Add Direction Streak as 4th signal:
  streak4_long: 4 consecutive positive returns (shift 1-4)
  streak4_short: 4 consecutive negative returns (shift 1-4)

DIRECTIONAL (unlike Wick Ratio which was non-directional and failed).
Orthogonal to all 3 existing signals:
  - GK: volatility structure (2nd moment H/L/O/C)
  - Skew: distribution asymmetry (3rd moment)
  - RetSign: proportion counting (% positive)
  - Streak: consecutive run-length (sequential pattern)

Params: streak_len=4 (standard "trend cluster" definition, pre-fixed)
"""
...
def calc(df):
    ...
    # --- Signal 4: Direction Streak ---
    # 4 consecutive positive/negative returns, all shifted by 1
    d["up"]=(d["ret"]>0).astype(float)
    d["dn"]=(d["ret"]<0).astype(float)
    # streak4_long: ret[i-1]>0 AND ret[i-2]>0 AND ret[i-3]>0 AND ret[i-4]>0
    d["stk_l"]=(d["up"].shift(1)*d["up"].shift(2)*d["up"].shift(3)*d["up"].shift(4))==1
    d["stk_s"]=(d["dn"].shift(1)*d["dn"].shift(2)*d["dn"].shift(3)*d["dn"].shift(4))==1
    ...
    d["any_l"]=d["gk_comp"]|d["skew_l"]|d["sr_l"]|d["stk_l"]
    d["any_s"]=d["gk_comp"]|d["skew_s"]|d["sr_s"]|d["stk_s"]
...
def bt(df):
    ...
    STL=df["stk_l"].values;STS=df["stk_s"].values
    ...
    for i in range(W,len(df)-1):
        ...
        long_sig=(gkc or skl or srl or stl) and bl and sok
        short_sig=(gkc or sks or srs or sts) and bs and sok
...
if __name__=="__main__":
    print("="*60)
    print("  Round 21: Quad OR + Streak (GK+Skew+RetSign+Streak4)")
    print("="*60)
    df=load();df=calc(df)
    ...
    c_stl=(v["stk_l"]==True).sum();c_sts=(v["stk_s"]==True).sum()
    ...
    print(f"  Streak L/S: {c_stl}/{c_sts}")
    ...
    print(f"        gk_r.shift(1).rolling(100); skew.shift(1); sr.shift(1)")
    print(f"        streak: up.shift(1)*up.shift(2)*up.shift(3)*up.shift(4)")
    print(f"    [2] Entry=next bar open?: YES: nxo=O[i+1]")
    print(f"    [3] All rolling have shift(1)?: YES")
    print(f"    [4] Params pre-fixed?: YES: GK Champion, Skew R1, Sign R16, Streak=4 standard")
