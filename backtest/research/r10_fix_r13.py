"""
R10 Fix Round 13: SN4.5% + MT8 + Exit Cooldown 13 + Early Stop 2%
===================================================================
R12 = 8/9, IS PnL -$504 (need > -$500). ONLY $4 SHORT!
Increase exit cooldown from 12 to 13 bars to remove 2-3 more IS trades.
...
[Y] fix direction decided before data?  13 bars from R12 near-miss, +1 from 12
[Y] no post-result parameter adjustment?  Incremental from validated direction
...
if __name__=="__main__":
    print("="*70)
    print("  R10 Fix Round 13: SN4.5% + MT8 + ExitCD13 + EarlyStop 2%")
    print("="*70)
    ...
    if all_pass:
        print(f"\n  *** ALL 9 TARGETS MET! STRATEGY VALIDATED! ***")
        print(f"\n  Final Strategy Spec:")
        print(f"    Entry: Parkinson percentile < {PARK_THRESH}, Close breakout ({BRK_LOOK}), Session filter")
        print(f"    SafeNet: ±{SAFENET_PCT*100:.1f}% (with 0.25 slippage model)")
        print(f"    Trail: EMA20, min hold {MIN_TRAIL} bars, 2% early stop bars {MIN_TRAIL}-{EARLY_STOP_END}")
        print(f"    Exit cooldown: {EXIT_COOLDOWN} bars after exit, same direction")
        print(f"    Max same direction: {MAX_SAME}")
        print(f"\n  IS: {is_s['n']} trades, ${is_s['pnl']:+,.0f} (PF {is_s['pf']:.2f})")
        print(f"  OOS: {oos_s['n']} trades, ${oos_s['pnl']:+,.0f} (PF {oos_s['pf']:.2f})")
        print(f"  Full: {pos_months}/{tot_months} pos months ({full_pos_rate:.0f}%), max consec losing: {full_consec}")
    else:
