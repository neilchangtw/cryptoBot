"""
V10-L Round 5: Add EMA trend filter to GK breakout
R4 finding: NoES improves OOS from $1,580 to $2,624.
            But worst month -$333 and MDD $834 still fail.
            Losing months are bear/choppy (Jun/Sep/Oct/Nov).
Change: Add close > EMA(trend_len) filter to cut bear-month entries.

Also test: combining trend filter with ES1-5@2% (R4 #2, better risk profile)
"""
import sys, io
...
# EMAs (shift(1))
for span in [20, 40, 50, 60, 80, 100]:
...
def run_bt(df, p, start, end):
    gk_t = p["gk_thresh"]; bl = p["breakout"]; sn = p["safenet"]
    mh = p["min_hold"]; maxh = p["max_hold"]
    cd = p.get("exit_cd", 8); cap = p.get("entry_cap", 20)
    es_start = p.get("es_start", 0); es_end = p.get("es_end", 0)
    es_pct = p.get("es_pct", 0)
    trend_ema = p.get("trend_ema", 0)  # 0 = no trend filter
    bo_col = f"bo_{bl}"
    ...
    for i in range(start, end):
        ...
        # Trend filter
        if trend_ema > 0:
            ema_val = b[f"ema{trend_ema}"]
            if pd.isna(ema_val) or b["close"] <= ema_val:
                continue
...
# ─── Grid: Trend filter × base params ────────────────
# Test trend EMA + best R4 base params
grid = list(product(
    [0, 40, 50, 60, 80, 100],   # trend_ema (0=none=R4 baseline)
    [30, 40, 50],                # gk_thresh
    [12, 15],                    # breakout
    [3.5, 4.5],                  # safenet
    [7, 10],                     # min_hold
...
print(f"=== V10-L R5: Trend Filter ===")
print(f"Grid: {len(grid)} configs")
print()

results = []
for idx, (tr, gk, bl, sn, mh, maxh, (ess, ese, esp, esl)) in enumerate(grid):
    p = {"trend_ema":tr, "gk_thresh":gk, "breakout":bl, "safenet":sn,
         "min_hold":mh, "max_hold":maxh, "exit_cd":8, "entry_cap":20,
         "es_start":ess, "es_end":ese, "es_pct":esp}
    ...
    results.append({"p":p, "is":is_m, "oos":oos_m,
                     ...
                     "es_label":esl, "trend":tr})
    if (idx+1) % 100 == 0:
        print(f"  ... {idx+1}/{len(grid)}")

# ─── Analysis by trend ────────────────────────────────
print(f"\n{'='*65}")
print("RESULTS BY TREND FILTER")
print(f"{'='*65}")

trend_labels = {0: "No trend", 40: "EMA40", 50: "EMA50", 60: "EMA60", 80: "EMA80", 100: "EMA100"}
for tr_val in [0, 40, 50, 60, 80, 100]:
    sub = [r for r in results if r["trend"]==tr_val]
    ...
    print(f"\n[{trend_labels[tr_val]}] IS>0: {is_pos}/{len(sub)}, Both>0: {len(both)}, "
    ...
    if best:
        ...
        print(f"  Best: GK<{p['gk_thresh']} BL={p['breakout']} SN={p['safenet']} "
              f"mh={p['min_hold']} maxH={p['max_hold']} {best['es_label']}")
...
print(f"\n{'='*65}")
print(f"TOP 20 (IS>0 AND OOS>0)")
print(f"{'='*65}")
print(f"{'#':>2} {'TR':>4} {'GK':>3} {'BL':>3} {'SN':>4} {'mh':>3} {'mxh':>4} {'ES':>10} | "
      f"{'ISn':>3} {'IS$':>6} {'ISW':>4} | "
      f"{'On':>3} {'O$':>6} {'OWR':>4} {'OPF':>5} {'MDD':>5} {'PM':>5} {'WM':>6}")
for i, r in enumerate(all_both[:20], 1):
    p=r["p"]; s=r["is"]; o=r["oos"]
    pm=f"{o['pos_m']}/{o['tot_m']}"
    tr_s = f"E{p['trend_ema']}" if p['trend_ema']>0 else "none"
    print(f"{i:>2} {tr_s:>4} {p['gk_thresh']:>3} {p['breakout']:>3} {p['safenet']:>4} "
          f"{p['min_hold']:>3} {p['max_hold']:>4} {r['es_label']:>10} | "
          f"{s['n']:>3} {s['pnl']:>+6.0f} {s['wr']:>4.0f} | "
          f"{o['n']:>3} {o['pnl']:>+6.0f} {o['wr']:>4.0f} {o['pf']:>5.2f} "
          f"{o['mdd']:>5.0f} {pm:>5} {o['worst_m']:>+6.0f}")

# ─── Focus on configs passing worst_month >= -200 ────
wm_pass = [r for r in all_both if r["oos"]["worst_m"] >= -200]
print(f"\n{'='*65}")
print(f"CONFIGS WITH WORST MONTH >= -$200: {len(wm_pass)}")
if wm_pass:
    wm_pass.sort(key=lambda x: x["oos_pnl"], reverse=True)
    for i, r in enumerate(wm_pass[:10], 1):
        p=r["p"]; s=r["is"]; o=r["oos"]
        pm=f"{o['pos_m']}/{o['tot_m']}"
        tr_s = f"E{p['trend_ema']}" if p['trend_ema']>0 else "none"
        print(f"  {i:>2} TR={tr_s} GK<{p['gk_thresh']} BL={p['breakout']} SN={p['safenet']} "
              f"mh={p['min_hold']} maxH={p['max_hold']} {r['es_label']}: "
              f"IS ${s['pnl']:+.0f} | OOS ${o['pnl']:+.0f} WR={o['wr']:.0f}% "
              f"PF={o['pf']:.2f} MDD=${o['mdd']:.0f} PM={pm} WM=${o['worst_m']:+.0f}")

# ─── Focus on configs passing MDD <= 400 ─────────────
mdd_pass = [r for r in all_both if r["oos"]["mdd"] <= 400]
print(f"\nCONFIGS WITH MDD <= $400: {len(mdd_pass)}")
if mdd_pass:
    mdd_pass.sort(key=lambda x: x["oos_pnl"], reverse=True)
    for i, r in enumerate(mdd_pass[:10], 1):
        p=r["p"]; s=r["is"]; o=r["oos"]
        pm=f"{o['pos_m']}/{o['tot_m']}"
        tr_s = f"E{p['trend_ema']}" if p['trend_ema']>0 else "none"
        print(f"  {i:>2} TR={tr_s} GK<{p['gk_thresh']} BL={p['breakout']} SN={p['safenet']} "
...
# ─── All independent goals pass ──────────────────────
all_pass = [r for r in all_both if
            r["oos"]["pos_m"] >= min(8, r["oos"]["tot_m"]) and
            r["oos"]["worst_m"] >= -200 and
            r["oos"]["mdd"] <= 400]
print(f"\nALL INDEPENDENT GOALS PASS: {len(all_pass)}")

# ─── Top 3 detailed ──────────────────────────────────
for rank, r in enumerate(all_both[:3], 1):
    p=r["p"]; o=r["oos"]; s=r["is"]
    tr_s = f"EMA{p['trend_ema']}" if p['trend_ema']>0 else "none"
    print(f"\n{'─'*60}")
    print(f"[#{rank}] TR={tr_s} GK<{p['gk_thresh']} BL={p['breakout']} SN={p['safenet']}% "
          f"mh={p['min_hold']} maxH={p['max_hold']} {r['es_label']}")
...
print("R5: GK Breakout+Trend Filter+NoES -> see above")
