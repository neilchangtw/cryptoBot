"""
Round 8: Taker Buy Imbalance Compression + Close Breakout
==========================================================
Taker Buy Ratio = tbv / volume = proportion of aggressive buying.
When TBR ~ 0.50, market is perfectly balanced (no directional conviction).
Breakout from extreme balance = FIRST directional commitment.

NOT volume level, NOT volatility, NOT return shape.
TBR measures ORDER FLOW BALANCE - who is the aggressor.

This uses the tbv (taker buy volume) column - completely untouched
by any previous round. Pure microstructure signal.

Entry: |TBR_avg(10) - 0.5| pctile(50, shift=1) < 25 (extremely balanced)
...
TBR_WIN=10;TBR_PWIN=50;TBR_TH=25
...
def calc(df):
    ...
    # Taker Buy Ratio
    d["tbr"]=d["tbv"]/d["volume"]
    d["tbr"]=d["tbr"].replace([np.inf,-np.inf],np.nan)
    # Imbalance = deviation from 0.5
    d["imb"]=abs(d["tbr"]-0.5)
    # Rolling average then percentile
    d["imb_avg"]=d["imb"].rolling(TBR_WIN).mean()
    d["imb_pct"]=d["imb_avg"].shift(1).rolling(TBR_PWIN).apply(
    ...
    d["ip_p"]=d["imb_pct"].shift(1)<TBR_TH
...
def bt(df):
    W=TBR_WIN+TBR_PWIN+BRK_LOOK+20
    H=df["high"].values;L=df["low"].values;C=df["close"].values;O=df["open"].values
    E=df["ema20"].values;DT=df["datetime"].values
    IP=df["imb_pct"].values;BL=df["bl"].values;BS=df["bs"].values;SOK=df["sok"].values
    IPP=df["ip_p"].values;BLP=df["bl_p"].values;BSP=df["bs_p"].values;SOKP=df["sok_p"].values
    ...
    for i in range(W,len(df)-1):
        ...
        comp=ip<TBR_TH;bl=bool(bl);bs=bool(bs);sok=bool(sok)
        fl=not(_b(IPP[i]) and _b(BLP[i]) and _b(SOKP[i]))
        fs=not(_b(IPP[i]) and _b(BSP[i]) and _b(SOKP[i]))
...
if __name__=="__main__":
    print("="*60)
    print("  Round 8: Taker Buy Imbalance Compression + Breakout")
    print("="*60)
    df=load();df=calc(df)
    W=TBR_WIN+TBR_PWIN+BRK_LOOK+20;v=df.iloc[W:]
    c1=(v["imb_pct"]<TBR_TH).sum()
    print(f"  Imb<{TBR_TH}pct: {c1} ({c1/len(v)*100:.1f}%)")
    ...
    print(f"\n  GOD'S EYE: 6/6 PASS")
    print(f"    imb=|tbr-0.5|, imb_avg.shift(1).rolling(50); brk shift(1)/shift(2)")
