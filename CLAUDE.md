...
**實戰（目前的運行方式）**：VPS systemd 服務，git 部署。

```bash
ssh cryptobot@187.127.108.237
cd ~/cryptoBot && git pull && sudo systemctl restart cryptobot   # 更新
journalctl -u cryptobot -f                                       # 看日誌
...
│
│  ── 回測研究 ──
├── backtest/research/     # 330+ 支回測研究腳本（歷史研究用，不影響運行）
│
│  ── 其他 ──
...
| `alerts.log` | 錯誤/告警日誌（API 失敗、異常狀態；心跳自檢的「今日告警數」讀此檔） |

> 三個檔皆有輪替（RotatingFileHandler，單檔 5MB × 3 備份），不會無限成長。
...
> **2026-07-03~06 追加部署**：V29 策略健康度上線、錢包金額群組隱私遮罩、/help 分組、
> 心跳自檢重構（倉位同步 + SafeNet 掛單保護 + 資料新鮮度）、通知文案（運轉天數/進場帶 TP 價位）、
> get_positions 誤報修正、群組升級 supergroup 自動偵測、日誌輪替（最新 `9246e47`）。
> 本段保留作為變更說明與操作注意事項。
...
# analyze.py / run_backtest.py 輸出尾端自動附「策略證偽檢查」四項 HP%（edge_falsify.py）：
...
#   Edge 項「獲利中不觸紅」：窗口平均 >0 最低 🟡（弱≠死），合計轉虧才 🔴（歷史校準
...
python run_backtest.py --flat                      # 全程 200U/$4,000（關閉保證金歷史，= 研究基準數字）
...
| 指令 | 功能 |
|------|------|
| `/status`（`/pos`） | 倉位同步狀態（內部 vs Binance；餘額行群組不顯示） |
| `/bal`（`/balance`） | 帳戶餘額 + 未實現損益（**🔒 僅私聊**，群組拒答導向私聊） |
| `/pnl` | 今日 / 本月 / 近 7 天損益報表 |
| `/analysis 30`（`/stats`、`/report`） | 收益分析（WR/PF/最大回撤/出場分佈/L vs S/regime），數字=最近 N 天，不帶=全期間 |
| `/signal`（`/cond`、`/check`） | 即時開單條件檢查（L/S 每個 gate ✅/❌ + 可開單時段 + 即時 GK/regime） |
| `/trades` | 最近 5 筆交易 |
| `/alerts`（`/warn`） | 今日告警日誌 |
| `/cb` | 風控熔斷狀態 + **策略健康度**（V29） |
| `/pause` / `/resume` | 暫停 / 恢復開新倉（**管理員限定**，見 TELEGRAM_ADMIN_IDS） |
| `/cleanup`（`/clean`） | 清理孤兒倉位（**管理員限定**） |
| `/help` | 顯示指令列表（分查詢/控制兩組） |
...
> 通知會靜默失敗，需更新 .env。
>
> **錢包金額隱私（2026-07-06 起）**：所有含帳戶餘額的內容（進出場/心跳/開關機訊息的
> 金庫行、/status 餘額行）以 `wrap_private()` 標記 → **私聊照常顯示、發到群組（負數
> chat id）時自動整段剝除**；`/bal` 在群組中直接拒答導向私聊。實作：telegram_notify
> `[[HIDE]]` 遮罩，發送時按目標逐一套用。PnL 類數字（賺賠金額）不在隱藏範圍。
